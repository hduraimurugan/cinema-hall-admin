...
export function TeamInviteDialog({ open, onOpenChange, onSuccess, roles }) {
  const { halls } = useHall()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await teamService.inviteMember({
        ...
        halls: hallAssignments.filter((a) => a.hallId).map((a) => a.hallId),
      })
                ...
                {halls.map((hall) => {
                  ...
                  return (
                    <div key={hall.id} className="flex items-center gap-3 rounded-lg border border-border/50 p-3">
                      ...
                      />
                      <Label htmlFor={`hall-${hall.id}`} className="flex-1 text-sm cursor-pointer">
                        {hall.name}
                      </Label>
                      {assignment && (
                        <Select
