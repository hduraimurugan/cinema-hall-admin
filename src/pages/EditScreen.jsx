...
const EditScreen = ({ screenId, onBack, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (savedScreens && screenId) {
      ...
      if (screen) {
        ...
        // Extract row labels from seats
        const labels = {}
        screen.layout.seats.forEach((seat) => {
          const rowIndex = Number.parseInt(seat.id.split("-")[0])
          if (!labels[rowIndex]) {
            labels[rowIndex] = seat.row
          }
        })
        setRowLabels(labels)
      }
  ...
  const updateRowLabel = (rowIndex, label) => {
    setRowLabels((prev) => ({ ...prev, [rowIndex]: label }))
    // Update all seats in this row
    setLayout((prev) => ({
      ...prev,
      seats: prev.seats.map((seat) => {
        const seatRowIndex = Number.parseInt(seat.id.split("-")[0])
        return seatRowIndex === rowIndex ? { ...seat, row: label } : seat
      }),
    }))
  }
  ...
  const saveScreen = () => {
    ...
    const seatCounts = layout.seats.reduce(
      (acc, seat) => {
        if (seat.type in acc && !seat.isBlocked) {
          acc[seat.type]++
        }
        return acc
      },
    ...
    const updatedScreens = existingScreens.map((screen) => (screen.id === screenId ? screenData : screen))
  ...
  const resetLayout = () => {
    if (confirm("Are you sure you want to reset the entire layout?")) {
      // Reset to original state when component loaded
      const savedScreens = localStorage.getItem("cinema-screens")
      if (savedScreens && screenId) {
        const screens = JSON.parse(savedScreens)
        const screen = screens.find((s) => s.id === screenId)
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Modify your cinema screen layout</p>
        </div>
      </div>
                      ...
                      <div key={rowIndex} className="flex items-center gap-1 mb-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <div className="w-4 text-center">
                            <Input
                              value={rowLabels[rowIndex] || String.fromCharCode(65 + rowIndex)}
                              onChange={(e) => updateRowLabel(rowIndex, e.target.value)}
                              className="w-8 h-8 text-center text-xs p-0"
                              maxLength={2}
                            />
                          </div>
                        </div>
